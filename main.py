...
def load_qt_data(dataset):
    qt_tf = pd.read_csv(r'D:/Latest/T-GCN-master/data/merged_speed_road_data_travel_time_tt_matrix1.csv')    # sz_tf = pd.read_csv(r'/home/abid/Desktop/Progress_report/rush_hours_matrix.csv')
    return qt_tf
...
def att_GRU(_D, wts, bias):
    ...
    outcome, stt = tf.nn.static_rnn(cell_dfd, _D, dtype=tf.float32)

    res = tf.concat(outcome, axis=0)
    ...
    output = tf.reshape(outcome_e,shape=[-1,len_hist])
    output = tf.matmul(output, wts['output']) + bias['output']
    output = tf.reshape(output,shape=[-1,nodes_total,len_pred])
    output = tf.transpose(output, perm=[0,2,1])
    output = tf.reshape(output, shape=[-1,nodes_total])

    return output, outcome, stt, alp

def GRU(_D, model_weights, biases):
    ###
    def_cell = RNN(hidden_units_gru, num_nodes=nodes_total)
    grucell = tf.nn.rnn_cell.MultiRNNCell([def_cell], state_is_tuple=True)
    _D = tf.unstack(_D, axis=1)
    outcome, stt = tf.nn.static_rnn(grucell, _D, dtype=tf.float32)
    ...
    fout = tf.reshape(res,shape=[-1,len_hist])
    fout = tf.matmul(fout, model_weights['output']) + model_bias['output']
    fout = tf.reshape(fout,shape=[-1,nodes_total,len_pred])
    fout = tf.transpose(fout, perm=[0,2,1])
    fout = tf.reshape(fout, shape=[-1,nodes_total])
    return outcome, fout, stt
    # return outcome, res, stt, alp
...
def eval_measure(target,predicted):
    ...
    normF = la.norm(target-predicted,'fro')/la.norm(target,'fro')
    return rmse, mae, mape, r2, 1-normF
...
# # ### Robustness Analysis
# guass_noise = np.random.normal(0,0.2,size=ds.shape)
# # #guass_noise = np.random.poisson(16,size=ds.shape)
# scaler = MinMaxScaler()
# scaler.fit(guass_noise)
# noise = scaler.transform(guass_noise)
# ds1 = ds1 + guass_noise
...
if approach == 'att_GRU':
    prediction,_,_,_ = att_GRU(data, model_weights, model_bias)
if approach == 'GRU':
    prediction,_,_ = GRU(data, model_weights, model_bias)    
